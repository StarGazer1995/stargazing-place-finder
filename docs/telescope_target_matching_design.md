...
> **状态：Phase 1 ✅ 完成，Phase 2 ✅ 完成，Phase 3 设计中** | 上次更新：2026-07-06
...
| # | 任务 | 说明 |
|---|------|------|
| 3.1 | 地点 → 目标串联 | 分析结果中选一个地点 → 切望远镜模式 → 自动带入坐标 + 一键匹配 |
| 3.2 | 反向查询 | 选一个目标天体 → 地图上高亮适合拍它的低光污染区域 |
| 3.3 | 天气接入 | 结合 mcp-stargazing 天气数据，标注"今晚此目标可见时段" |
| 3.4 | 月相叠加 | 星图模式下显示月相信息，辅助判断拍摄窗口 |

**核心思路**：SPF 已有的"区域分析"结果（光污染低的地点列表）和 Phase 2 的"匹配引擎"（设备适配的天体排序）是两套独立流程。Phase 3 把它们串起来：

1. **3.1 地点→目标**：在分析结果卡片上增加"🔭 在此拍摄"按钮 → 切望远镜模式 → 自动设置观测坐标 → 一键匹配目标
2. **3.2 目标→地点**：在目标卡片上增加"📍 找拍摄地点" → 在地图上标出光污染等级低的区域
3. **3.3 天气**：在匹配结果中显示当前天气摘要（云量/风速/可见度），标注"今晚适合拍摄"
4. **3.4 月相**：在 Aladin 星图侧显示月相卡片（亮度/相位），辅助判断月光干扰
