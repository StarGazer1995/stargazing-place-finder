...
> **状态：Phase 1 ✅ 完成，Phase 2 ✅ 完成，Phase 3 ✅ 完成，Phase 4 🚧 进行中（拍摄计划 ✅、马赛克 🚧）** | 上次更新：2026-07-07
...
> **依赖**：Phase 2 ✅ 完成
> **状态**：✅ 完成（3.1 + 3.4，3.3 搁置）

| # | 任务 | 状态 | 复杂度 | 涉及仓库 |
|---|------|------|--------|---------|
| 3.1 | 地点 → 目标串联 | ✅ 方案已定 | 低 | SPF 前端 |
| 3.2 | ~~反向查询~~ | ❌ 砍掉 | — | — |
| 3.3 | 天气服务下沉 + 展示 | ⏸️ Pending | 中 | stargazing-core + SPF |
| 3.4 | 月相叠加 | ✅ 方案已定 | 中 | stargazing-core + SPF 前端 |

**推荐执行顺序**：3.1 → 3.4 → 3.3（待天气瓦片解决后）

---

#### 3.1 地点 → 目标串联 ✅

**目标**：在分析结果 popup 和点地图 popup 中增加入口，一键跳转到该地点的望远镜匹配。

**数据流**：

```
分析结果 marker popup / 点地图 popup   望远镜模式
┌─────────────────────────┐          ┌─────────────────────┐
│ 📍 XX 观星点   ⭐8.5/10 │          │ 🔭 RedCat51         │
│ 🌃 Bortle Class 2       │          │ 📍 当前地点：XX 观星点│
│ 🚗 道路可达              │          │                     │
│                          │          │ [🎯 匹配拍摄目标]    │
│  [🔭 在此拍摄] ──────────┼──点击──→│                     │
└─────────────────────────┘          └─────────────────────┘
...
- 在 **三种 popup** 中统一添加 "🔭 在此拍摄" 按钮：
  - `createDetailedPopupContent()` — 点地图 API 成功（光污染详细数据）
  - `createPopupContent()` — 点地图回退（本地 Bortle 数据）
  - `createStargazingMarker()` — 画框分析结果（完整地点信息）
- 点击 → 调用 `jumpToTelescopeMode(lat, lng, name)` 切换到望远镜模式
- 自动将 `lat` / `lng` 填入观测坐标，地点名（如有）显示在面板顶部
- **不自动触发匹配**，让用户确认设备参数后手动点击

**附带修复：保留分析状态**：

当前 `syncPanelVisibility()` 在进入望远镜模式时错误地销毁了画框控件和分析标记（`drawControl = null; drawnItems = null; currentPolygon = null`）。由于地图已经 `display: none` 隐藏，这些销毁操作是多余的。修复：去掉望远镜模式分支中的这四行，确保切回地图时分析结果完好无损。

**改动文件**：

| 文件 | 改动 |
|------|------|
| `app.js` `syncPanelVisibility()` | 望远镜模式分支中删除 drawControl/drawnItems/currentPolygon 销毁代码 |
| `app.js` `createDetailedPopupContent()` | 加按钮 |
| `app.js` `createPopupContent()` | 加按钮 |
| `app.js` `createStargazingMarker()` | 加按钮 |
| `app.js` 新增 | `jumpToTelescopeMode(lat, lon, name)` |
| `template.html` | 望远镜面板加地点名显示行 |
| `styles.css` | 按钮样式 |
...
天体在天上，不在地上。拍摄天体只需要低光污染地点，SPF 本身就是干这个的。不存在"目标天体→特定地理地点"的映射关系。砍掉。

---

#### 3.3 天气服务下沉 + 展示 ⏸️ Pending

**目标**：将 MCP 的天气服务下沉到 stargazing-core，使 MCP 和 SPF 共享同一套天气数据源。在 SPF popup 和匹配结果中展示天气摘要。

**架构**：

```
stargazing-core _weather/
├── models.py          ← Pydantic 模型（从 MCP schemas/weather.py 迁移）
├── open_meteo.py      ← 从 MCP providers/ 迁移
├── qweather.py        ← 从 MCP providers/ 迁移
├── wttr.py            ← 从 MCP providers/ 迁移
├── service.py         ← 多 provider 聚合
└── qweather_auth.py   ← 从 MCP qweather_interaction.py 迁移

MCP weather/impl.py    → 变成薄 wrapper，调用 core
SPF 后端                → 调用 core 取天气
```

**展示落脚点**：
- 点地图 popup：加天气摘要（云量/风速/温度）
- 分析结果 marker popup：每个地点显示天气
- 望远镜匹配结果：未来 3-7 天云量卡片

**Pending 原因**：缺少可视化天气瓦片。用户需要先在地图上看到"哪片区域天气好"才能决定去哪。SPF 本身不是天气预报 App，但天气信息对观星决策至关重要。

**解决方向**：
- 找到无需 API key 的云量瓦片（如 RainViewer 只覆盖降水）
- 或接入开放式天气瓦片服务
- 在此之前，3.3 搁置
...
**目标**：匹配结果中显示月相信息，基于**整个拍摄窗口（昏影→晨曦）**的月光状态给出拍摄建议。

**当前缺口**：`match_telescope_targets` 内部已计算 `moon_info`、`moon_alt`/`moon_az`，但全部没有暴露到返回值。

**Step 1 — stargazing-core 暴露月相字段**：

在返回 dict 中（顶层，不在每个 target 中重复）新增：

```python
'moon_illumination': moon_info['illumination'],     # 0.0-1.0
'moon_phase': moon_info['phase_name'],              # "Waxing Gibbous" 等
'moon_altitude_curve': [...],                        # 和目标曲线同一时间轴
'moon_always_down': True/False,                     # 整晚都在地平线下？
'moon_dark_fraction': 0.78,                         # 无月时间占比（0-1）
```

月亮高度角曲线复用已有的 15 分钟步长时间网格（昏影→晨曦）。
...
|------|------|
| 亮度 ≤ 30% 或整晚在地平线下 | 🟢 无干扰 |
| 亮度 > 30% 且暗夜窗口 > 4 小时 | 🟡 可接受 |
| 亮度 > 30% 且暗夜窗口 < 2 小时 | 🔴 高干扰 |
| 中间状态 | 🟠 中干扰 |
...
| 仓库 | 文件 | 改动 |
|------|------|------|
| stargazing-core | `_filtering.py` | 返回 dict 加 5 个月相字段 + 月亮高度角曲线 |
| stargazing-core | `tests/test_filtering.py` | 验证月相字段存在 |
| SPF | `app.js` `renderTargetResults()` | 解析月相 + 渲染卡片 |
| SPF | `template.html` | 月相卡片 DOM |
...
| # | 功能 | 状态 | 说明 |
|---|------|------|------|
| 4.1 | 马赛克规划 | 🚧 PR #82 | 超大天体 → 自动拼接网格（Aladin 多 FOV + 重叠率滑块） |
| 4.2 | 曝光计算器 | ⬜ | 口径+焦比+目标表面亮度 → 建议子帧时长和总积分时间 |
| 4.3 | 多晚规划 | ⬜ | 未来一周每晚最佳拍摄目标排序 |
| 4.4 | 配置持久化 | ⬜ | 用户保存自己的设备配置 |
| — | **拍摄计划** | ✅ | 单晚逐分钟拍摄排程（#81） |

#### 拍摄计划（shooting plan）✅

实现为 stargazing-core 的 `generate_shooting_schedule` 函数 + SPF 前端面板：

- **算法**：`np.interp` 线性插值高度角 + `np.nanargmax` 逐分钟最优目标 + `np.diff` run 合并
- **API**：`POST /api/telescope/plan` — 返回 targets + moon + timed shooting slots
- **前端**：可折叠面板，每 slot 显示目标名、类型、FOV 适配、时段、高度角上下限
- **交互**：点击 slot → Aladin 跳转目标 + 高度角图表
- **月相感知**：月落后延迟拍摄、窄带推荐、未分配时间警告
