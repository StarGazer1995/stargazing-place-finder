...
class TestGisQueryService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.postgis_patcher = patch(
            'gis_service.query_service.PostgisBackend'
        )
        self.overpass_patcher = patch(
            'gis_service.query_service.OverpassBackend'
        )
    ...
    def test_road_connectivity_postgis_accessible(self):
        ...
        self.mock_postgis.query_road_connectivity.assert_called_once_with(
            39.9042, 116.4074, 10.0, 'drive'
        )
    ...
    def _make_mock_element(self, osm_id: int, lat: float, lon: float,
    ...
    def test_find_elevation_delegates(self):
        with patch.object(self.mock_postgis, 'find_elevation_at_point',
                          return_value=500.0):
            service = self._make_service(db_config={'host': 'localhost'})
