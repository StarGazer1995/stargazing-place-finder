...
class BatchElevationQuery:
    ...
    def query_elevations(
        self, coordinates: List[Tuple[float, float]], names: Optional[List[str]] = None, use_batch_query: bool = True
        ...
        try:
            ...
            logger.info(
                f"查询完成: {successful_count}/{len(coordinates)} 成功, "
    ...
    def _batch_query(self, coordinates: List[Tuple[float, float]], names: List[str]) -> List[ElevationResult]:
        ...
        for i in range(0, len(coordinates), self.batch_size):
            ...
            logger.debug(f"处理批次 {i // self.batch_size + 1}: {len(batch_coords)} 个点")
    ...
    def _execute_batch_query(self, coordinates: List[Tuple[float, float]], names: List[str]) -> List[ElevationResult]:
        ...
        try:
            ...
            for i, (lat, lon) in enumerate(coordinates):
                cursor.execute(
                    "INSERT INTO temp_elevation_points (lat, lon, name) VALUES (%s, %s, %s)", (lat, lon, names[i])
                )
            ...
            for row in query_results:
                (point_id, lat, lon, name, elevation, source_name, distance, feature_type) = row
...
def batch_query_elevations(
    coordinates: List[Tuple[float, float]], db_config: Dict, names: Optional[List[str]] = None, batch_size: int = 50
) -> List[ElevationResult]:
    """
    便捷的批量海拔查询函数
